...
def create_model(num_classes, pretrain=False):
    model = mpunetx(pretrain_backbone=True)
...
def main(args):
    ...
    # 用来保存训练以及验证过程中信息
    results_file = "./log/results{}.txt".format(datetime.datetime.now().strftime("%Y%m%d-%H%M%S"))
...
def parse_args():
    ...
    parser.add_argument('--eval_interval', default=1, type=int)  # 每多少轮验证一次
