...
class Dataset(Dataset):
    def __init__(self, root: str, train: bool = True, transforms=None):
        ...
        if train:
            self.image_root = os.path.join(root, "img_dir", "train")
            self.mask_root = os.path.join(root, "ann_2", "train")
        else:
            self.image_root = os.path.join(root, "img_dir", "val")
            self.mask_root = os.path.join(root, "ann_2", "val")
